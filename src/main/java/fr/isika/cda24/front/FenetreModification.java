...
public class FenetreModification extends BorderPane {
	...
	public FenetreModification(ArbreBinaireRecherche arbreImporte, ListeStagiaire listeImportee,
		...
		Label labelTitre = new Label("           	  Modification :");
		...
		modifierBtn.setFont(Font.font("Arial", FontWeight.BOLD, 16));
		// fixe la taille du bouton
		modifierBtn.setPrefHeight(45);
		modifierBtn.setPrefWidth(300);
		Image imageRecherche;
		try {
			imageRecherche = new Image(new FileInputStream("src/image/pen.png"));
			...
			// bouton
			imageViewRech.setFitWidth(modifierBtn.getPrefWidth() - 10);
			imageViewRech.setFitHeight(modifierBtn.getPrefHeight() - 10);
			modifierBtn.setGraphic(imageViewRech);
			modifierBtn.setGraphicTextGap(20);
		...
		modifierBtn.setBackground(bgBtnRech);
		modifierBtn.setEffect(dropShadow);
		...
		gridPane.add(modifierBtn, 1, 11, 1, 2);
...
// Action sur les boutons
		...
		modifierBtn.setOnAction(event -> {
			...
			if (nomTextField.getText().isEmpty()) {
				...
				return;
			} else if (nomTextField.getLength() > Stagiaire.TAILLE_NOM_MAX) {
				Alert alertSelec = new Alert(Alert.AlertType.WARNING);
				DialogPane dialogPaneSelec = alertSelec.getDialogPane();
				dialogPaneSelec.setStyle("-fx-font-family: Arial");
				...
				alertSupp.setTitle("Confirmation de modification");
				alertSupp.setHeaderText("Êtes-vous sûr de vouloir modifier ce stagiaire ?");
				...
				if (result.get() == ButtonType.OK) {
					...
					String departement = departementTextField.getText().isEmpty() ? null
